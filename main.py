...
def calculator_distance(para) :
    ...
    mycursor.execute("SELECT lon, lat FROM restaurant")
    ...
    for i in range(len(myResult)):
        if (haversine(para, location[i], unit = 'km')) <= 0.5:
    ...
    def __init__(self, parent, controller):
        ...
        def category_search():
            ...
            if self.selected_category.get() != "None" :
                category_subquery = "Select category_id FROM category WHERE category='"+str(self.selected_category.get()+"'")
            else:
                category_subquery = "Select DISTICT(category_id) FROM category" # question


        # Define the number of categories per row
        categories_per_row = 5
        for index, category in enumerate(categories):
            radiobutton = tk.Radiobutton(category_frame, text=category, variable= self.selected_category, value=category, command = category_search) #quesition
    ...
    def perform_search(self):
        ...
        if not input_text:
            ...
            try:
                ...
                mycursor.execute("SELECT restaurant_id FROM restaurant\
                                 WHERE restaurant_id IN " + str(tuple(restaurant_list)) 
                                 + "AND category_id IN "+ str(tuple(category_subquery))) # question
                restaurnat_Result = mycursor.fetchall()
                restaurant = list(restaurnat_Result)
        ...
        if self.go == 1:
            ...
            search_page.perform_search(lat,lon, self.selected_category.get(), english, kids)
...
class SearchResult(tk.Frame):
    ...
    def perform_search(self, lat, lon, category, english, kids): # question
        ...
        # self.filtered_data = self.controller.database # question
